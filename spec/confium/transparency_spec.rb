...
RSpec.describe Confium::Transparency::MerkleTree do
  ...
  it "rejects artifact_hash inputs that aren't 32 bytes" do
    ...
    end.to raise_error(ArgumentError, /artifact_hash must be exactly 32 bytes/)
  end
end
...
RSpec.describe Confium::Transparency::MerkleTree, '#verify_consistency' do
  def build_tree_with_snapshots(count)
  ...
  it 'rejects short old_root bytes' do
    ...
    end.to raise_error(ArgumentError, /old_root must be 32 bytes/)
  end
  ...
  it 'rejects short new_root bytes' do
    ...
    end.to raise_error(ArgumentError, /new_root must be 32 bytes/)
  end
  ...
  it 'rejects short proof entries' do
    ...
    end.to raise_error(ArgumentError, /proof entries must be 32 bytes/)
  end
end
...
RSpec.describe Confium::Transparency::InclusionProof, '#verify_with_leaf' do
  ...
  it 'rejects short leaf hash' do
    ...
    end.to raise_error(ArgumentError, /leaf_hash must be exactly 32 bytes/)
  end
  ...
  it 'rejects short root' do
    ...
    end.to raise_error(ArgumentError, /root must be exactly 32 bytes/)
  end
end
