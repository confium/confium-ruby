...
`aarch64-linux`, `x86_64-darwin`, and `arm64-darwin` — each carries
one extension per Ruby C-ABI window (3.1, 3.2, and 3.3+), so
installation on those platforms needs only:

- Ruby >= 3.1

Windows has no pre-compiled gems; see
[the README](https://github.com/confium/confium-ruby#prerequisites)
for the current Windows status.
...
the gem itself. Source builds require Rust stable 1.85+ and a C
toolchain (cc, make) — that's it, no Botan, no OpenSSL, no Boost;
the Confium engine ships as static Rust code:
...
refuses to compile without Rust stable 1.85+.
